...
import { registerUser } from "../Servers/ProducteServer"
  ...
  const handleSubmit = async  (event: FormEvent<HTMLFormElement>) => {
    ...
    await registerUser(form.name, form.email, form.password)
      .then(response => {
        ...
        toast.success("Registration successful! Welcome to Sci-Fai Store.")
        navigate("/"); 
      })
      .catch(error => {
          ...
          <div className="hidden lg:flex flex-col justify-between gap-8 bg-gradient-to-br from-orange-500 via-pink-500 to-violet-600 p-10 text-white">
            <div>
              ...
              <h2 className="mt-8 text-4xl font-extrabold leading-tight">
                Create a beautiful account and shop smarter.
              </h2>
              <p className="mt-4 max-w-md text-sm text-orange-100/90">
                Join the Sci-Fai Store family to save favorites, track orders, and access exclusive offers on your favorite products.
              </p>
            </div>

            <div className="space-y-4">
              <div className="rounded-3xl border border-white/20 bg-white/10 p-6 shadow-lg shadow-black/10 backdrop-blur-sm">
                <p className="text-sm uppercase tracking-[0.2em] text-orange-100">Fast checkout</p>
                <p className="mt-3 text-base text-white/90">Save your details and buy in just a few clicks.</p>
              </div>
              <div className="rounded-3xl border border-white/20 bg-white/10 p-6 shadow-lg shadow-black/10 backdrop-blur-sm">
                <p className="text-sm uppercase tracking-[0.2em] text-orange-100">Personalized picks</p>
                <p className="mt-3 text-base text-white/90">Get product suggestions tailored for your style.</p>
              </div>
              <div className="rounded-3xl border border-white/20 bg-white/10 p-6 shadow-lg shadow-black/10 backdrop-blur-sm">
                <p className="text-sm uppercase tracking-[0.2em] text-orange-100">Exclusive deals</p>
                <p className="mt-3 text-base text-white/90">Unlock special offers and early access to new drops.</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-8 py-10 sm:px-12 sm:py-14">
            <div className="mb-8">
              ...
              <p className="mt-4 max-w-xl text-sm text-slate-500 sm:text-base">
                Fill in your details to start shopping the latest products, save favorites, and enjoy fast checkout.
              </p>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <button
                ...
                className="mt-4 w-full rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold uppercase tracking-[0.12em] text-slate-700 shadow-sm transition duration-200 hover:bg-slate-50"
              >
                Continue as Guest
              </button>
            </form>
            ...
            <p className="mt-6 text-center text-sm text-slate-500">
              ...
              to="/login" className="font-semibold text-orange-600 hover:text-orange-700">
                Login
              </Link>
            </p>
